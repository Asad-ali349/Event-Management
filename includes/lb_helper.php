...
class LicenseBoxAPI{
	...
	public function __construct(){ 
		$this->product_id = 'B51C74C1';
		$this->api_url = 'http://projects.indratech.in/qdrmcydfas/';
		$this->api_key = '0D8215AFEE3D011E5DD4';
	...
	public function verify_license($time_based_check = false, $license = false, $client = false){
		...
		if($time_based_check && $this->verification_period > 0){
			...
			$today = date('d-m-Y');
			if(empty($_SESSION["4e78613bfb3482c"])){
				$_SESSION["4e78613bfb3482c"] = '00-00-0000';
			}
			if($type == 1){
				$type_text = '1 day';
			}elseif($type == 3){
			...
			if(strtotime($today) >= strtotime($_SESSION["4e78613bfb3482c"])){
				$get_data = $this->call_api(
					'POST',
